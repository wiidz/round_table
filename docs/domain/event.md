...
| **FreeDialogueQuestionAsked** | Participant / Principal | 自由对话提问（`principal_mediated` 时由 Moderator 转达；含 optional `token_usage`） |
...
| **ConfirmationApproved** | Principal | Principal 批准，进入最终结论（optional `item_notes`） |
| **ConfirmationRejected** | Principal | Principal 驳回，附 Feedback 和/或 `item_notes` |

...
| **轮间摘要** | 辩论轮结束后 `ModeratorSummarized`（提炼要点，非复制粘贴） |
| **自由对话** | Round 1 完成后 orchestrate Q&A（`FreeDialogue*` 事件） |
...
| D-Mod03 | **RoundCompleted** 写发言汇总；辩论轮 1+ 另发 **ModeratorSummarized** 提炼摘要 |
...
| 编号 | 说明 |
|------|------|
| — | Round 1 后插入 Free Dialogue，再发 Round 1 的 `ModeratorSummarized` |
| — | v0.1 Moderator 摘要为规则提炼（`engine/moderator.go`），非 LLM |
